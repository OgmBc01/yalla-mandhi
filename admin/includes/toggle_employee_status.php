...
try {
    $stmt = $connection->prepare(
        "UPDATE users SET is_active = ? WHERE id = ? AND role IN ('employee', 'admin', 'super-admin')"
    );
    ...
    if ($stmt->affected_rows === 0) {
        echo json_encode(['success' => false, 'message' => 'Employee not found or already has this status']);
    } else {
